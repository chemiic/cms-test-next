...
import removeProduct from "../../../actions/Api/removeProduct";
...
const TableItem: FC<TableItemProps> = ({product}) => {
    ...
    const onRemove: MouseEventHandler<HTMLButtonElement> = async (e)=>{
        e.preventDefault()
        const target = e!.target as HTMLElement
        const id = target.closest('button')!.getAttribute('id')
        const result = await removeProduct(id!)
        if(result.type == 'error'){
            onErrorOpen(`${result.res}`)
        } else{
            toast(`Товар успешно Удален: ${result.res}`, {
                position: "top-right",
                autoClose: 2000,
    ...
    return(
        <tr className="product w-full bg-white border-b border-solid text-left font-semibold">
            ...
            <td>{ product.count }</td>
            <td>${ product.price }</td>
            <td>$ {product.discount === 0 ? product.price * product.count : ((product.price * product.discount) - (product.price * product.count) * (product.discount / 100))}</td>

            <td className="flex items-center justify-end gap-1 h-[60px] pr-2">
                <button
